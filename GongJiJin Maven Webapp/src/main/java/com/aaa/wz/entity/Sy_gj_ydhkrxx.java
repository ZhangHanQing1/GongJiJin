package com.aaa.wz.entity;

import java.math.BigDecimal;
import java.util.Date;

public class Sy_gj_ydhkrxx {
    private Integer hkrzj;

    private Integer ydtqspzj;

    private Integer grbh;

    private String hkrxxbh;

    private String hkrdwzh;

    private String hkrgrzh;

    private String hkrxm;

    private String hkrzjhm;

    private BigDecimal hkryxj;

    private String cjr3;

    private Date cjsj3;

    private String dkzh;

    public Integer getHkrzj() {
        return hkrzj;
    }

    public void setHkrzj(Integer hkrzj) {
        this.hkrzj = hkrzj;
    }

    public Integer getYdtqspzj() {
        return ydtqspzj;
    }

    public void setYdtqspzj(Integer ydtqspzj) {
        this.ydtqspzj = ydtqspzj;
    }

    public Integer getGrbh() {
        return grbh;
    }

    public void setGrbh(Integer grbh) {
        this.grbh = grbh;
    }

    public String getHkrxxbh() {
        return hkrxxbh;
    }

    public void setHkrxxbh(String hkrxxbh) {
        this.hkrxxbh = hkrxxbh == null ? null : hkrxxbh.trim();
    }

    public String getHkrdwzh() {
        return hkrdwzh;
    }

    public void setHkrdwzh(String hkrdwzh) {
        this.hkrdwzh = hkrdwzh == null ? null : hkrdwzh.trim();
    }

    public String getHkrgrzh() {
        return hkrgrzh;
    }

    public void setHkrgrzh(String hkrgrzh) {
        this.hkrgrzh = hkrgrzh == null ? null : hkrgrzh.trim();
    }

    public String getHkrxm() {
        return hkrxm;
    }

    public void setHkrxm(String hkrxm) {
        this.hkrxm = hkrxm == null ? null : hkrxm.trim();
    }

    public String getHkrzjhm() {
        return hkrzjhm;
    }

    public void setHkrzjhm(String hkrzjhm) {
        this.hkrzjhm = hkrzjhm == null ? null : hkrzjhm.trim();
    }

    public BigDecimal getHkryxj() {
        return hkryxj;
    }

    public void setHkryxj(BigDecimal hkryxj) {
        this.hkryxj = hkryxj;
    }

    public String getCjr3() {
        return cjr3;
    }

    public void setCjr3(String cjr3) {
        this.cjr3 = cjr3 == null ? null : cjr3.trim();
    }

    public Date getCjsj3() {
        return cjsj3;
    }

    public void setCjsj3(Date cjsj3) {
        this.cjsj3 = cjsj3;
    }

    public String getDkzh() {
        return dkzh;
    }

    public void setDkzh(String dkzh) {
        this.dkzh = dkzh == null ? null : dkzh.trim();
    }
}