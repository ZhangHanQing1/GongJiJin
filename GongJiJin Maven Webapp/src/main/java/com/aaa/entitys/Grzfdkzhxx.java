package com.aaa.entitys;

import java.math.BigDecimal;
import java.util.Date;
import java.util.HashSet;
import java.util.Set;

/**
 * Grzfdkzhxx entity. @author MyEclipse Persistence Tools
 */

public class Grzfdkzhxx implements java.io.Serializable {

	// Fields

	private Integer dkzj;
	private String dkzh;
	private String jkhtbh;
	private String dkfxdj;
	private BigDecimal dkffe;
	private Date dkffrq;
	private BigDecimal dkye;
	private BigDecimal zxll;
	private BigDecimal llfdbl;
	private BigDecimal dkqs;
	private BigDecimal dqjhhkje;
	private BigDecimal dqjhghbj;
	private BigDecimal dqjhghlx;
	private BigDecimal dqyhze;
	private BigDecimal dqyhbj;
	private BigDecimal dqyhlx;
	private BigDecimal dqyhfx;
	private Date dkjqrq;
	private BigDecimal hsbjze;
	private BigDecimal hslxze;
	private BigDecimal fxze;
	private BigDecimal tqghbjze;
	private BigDecimal yqbj;
	private BigDecimal yqlx;
	private BigDecimal ljyqqs;
	private String shzt;
	private String sfzx;
	private String kjhskmbh;
	private BigDecimal wdbh3;
	private String dklrlx;
	private Set syDkZdhdlbs = new HashSet(0);
	private Set syDkYdtqsps = new HashSet(0);
	private Set grzfdkjkhtxxes = new HashSet(0);

	// Constructors

	/** default constructor */
	public Grzfdkzhxx() {
	}

	/** full constructor */
	public Grzfdkzhxx(String dkzh, String jkhtbh, String dkfxdj, BigDecimal dkffe, Date dkffrq, BigDecimal dkye,
			BigDecimal zxll, BigDecimal llfdbl, BigDecimal dkqs, BigDecimal dqjhhkje, BigDecimal dqjhghbj,
			BigDecimal dqjhghlx, BigDecimal dqyhze, BigDecimal dqyhbj, BigDecimal dqyhlx, BigDecimal dqyhfx,
			Date dkjqrq, BigDecimal hsbjze, BigDecimal hslxze, BigDecimal fxze, BigDecimal tqghbjze, BigDecimal yqbj,
			BigDecimal yqlx, BigDecimal ljyqqs, String shzt, String sfzx, String kjhskmbh, BigDecimal wdbh3,
			String dklrlx, Set syDkZdhdlbs, Set syDkYdtqsps, Set grzfdkjkhtxxes) {
		this.dkzh = dkzh;
		this.jkhtbh = jkhtbh;
		this.dkfxdj = dkfxdj;
		this.dkffe = dkffe;
		this.dkffrq = dkffrq;
		this.dkye = dkye;
		this.zxll = zxll;
		this.llfdbl = llfdbl;
		this.dkqs = dkqs;
		this.dqjhhkje = dqjhhkje;
		this.dqjhghbj = dqjhghbj;
		this.dqjhghlx = dqjhghlx;
		this.dqyhze = dqyhze;
		this.dqyhbj = dqyhbj;
		this.dqyhlx = dqyhlx;
		this.dqyhfx = dqyhfx;
		this.dkjqrq = dkjqrq;
		this.hsbjze = hsbjze;
		this.hslxze = hslxze;
		this.fxze = fxze;
		this.tqghbjze = tqghbjze;
		this.yqbj = yqbj;
		this.yqlx = yqlx;
		this.ljyqqs = ljyqqs;
		this.shzt = shzt;
		this.sfzx = sfzx;
		this.kjhskmbh = kjhskmbh;
		this.wdbh3 = wdbh3;
		this.dklrlx = dklrlx;
		this.syDkZdhdlbs = syDkZdhdlbs;
		this.syDkYdtqsps = syDkYdtqsps;
		this.grzfdkjkhtxxes = grzfdkjkhtxxes;
	}

	// Property accessors

	public Integer getDkzj() {
		return this.dkzj;
	}

	public void setDkzj(Integer dkzj) {
		this.dkzj = dkzj;
	}

	public String getDkzh() {
		return this.dkzh;
	}

	public void setDkzh(String dkzh) {
		this.dkzh = dkzh;
	}

	public String getJkhtbh() {
		return this.jkhtbh;
	}

	public void setJkhtbh(String jkhtbh) {
		this.jkhtbh = jkhtbh;
	}

	public String getDkfxdj() {
		return this.dkfxdj;
	}

	public void setDkfxdj(String dkfxdj) {
		this.dkfxdj = dkfxdj;
	}

	public BigDecimal getDkffe() {
		return this.dkffe;
	}

	public void setDkffe(BigDecimal dkffe) {
		this.dkffe = dkffe;
	}

	public Date getDkffrq() {
		return this.dkffrq;
	}

	public void setDkffrq(Date dkffrq) {
		this.dkffrq = dkffrq;
	}

	public BigDecimal getDkye() {
		return this.dkye;
	}

	public void setDkye(BigDecimal dkye) {
		this.dkye = dkye;
	}

	public BigDecimal getZxll() {
		return this.zxll;
	}

	public void setZxll(BigDecimal zxll) {
		this.zxll = zxll;
	}

	public BigDecimal getLlfdbl() {
		return this.llfdbl;
	}

	public void setLlfdbl(BigDecimal llfdbl) {
		this.llfdbl = llfdbl;
	}

	public BigDecimal getDkqs() {
		return this.dkqs;
	}

	public void setDkqs(BigDecimal dkqs) {
		this.dkqs = dkqs;
	}

	public BigDecimal getDqjhhkje() {
		return this.dqjhhkje;
	}

	public void setDqjhhkje(BigDecimal dqjhhkje) {
		this.dqjhhkje = dqjhhkje;
	}

	public BigDecimal getDqjhghbj() {
		return this.dqjhghbj;
	}

	public void setDqjhghbj(BigDecimal dqjhghbj) {
		this.dqjhghbj = dqjhghbj;
	}

	public BigDecimal getDqjhghlx() {
		return this.dqjhghlx;
	}

	public void setDqjhghlx(BigDecimal dqjhghlx) {
		this.dqjhghlx = dqjhghlx;
	}

	public BigDecimal getDqyhze() {
		return this.dqyhze;
	}

	public void setDqyhze(BigDecimal dqyhze) {
		this.dqyhze = dqyhze;
	}

	public BigDecimal getDqyhbj() {
		return this.dqyhbj;
	}

	public void setDqyhbj(BigDecimal dqyhbj) {
		this.dqyhbj = dqyhbj;
	}

	public BigDecimal getDqyhlx() {
		return this.dqyhlx;
	}

	public void setDqyhlx(BigDecimal dqyhlx) {
		this.dqyhlx = dqyhlx;
	}

	public BigDecimal getDqyhfx() {
		return this.dqyhfx;
	}

	public void setDqyhfx(BigDecimal dqyhfx) {
		this.dqyhfx = dqyhfx;
	}

	public Date getDkjqrq() {
		return this.dkjqrq;
	}

	public void setDkjqrq(Date dkjqrq) {
		this.dkjqrq = dkjqrq;
	}

	public BigDecimal getHsbjze() {
		return this.hsbjze;
	}

	public void setHsbjze(BigDecimal hsbjze) {
		this.hsbjze = hsbjze;
	}

	public BigDecimal getHslxze() {
		return this.hslxze;
	}

	public void setHslxze(BigDecimal hslxze) {
		this.hslxze = hslxze;
	}

	public BigDecimal getFxze() {
		return this.fxze;
	}

	public void setFxze(BigDecimal fxze) {
		this.fxze = fxze;
	}

	public BigDecimal getTqghbjze() {
		return this.tqghbjze;
	}

	public void setTqghbjze(BigDecimal tqghbjze) {
		this.tqghbjze = tqghbjze;
	}

	public BigDecimal getYqbj() {
		return this.yqbj;
	}

	public void setYqbj(BigDecimal yqbj) {
		this.yqbj = yqbj;
	}

	public BigDecimal getYqlx() {
		return this.yqlx;
	}

	public void setYqlx(BigDecimal yqlx) {
		this.yqlx = yqlx;
	}

	public BigDecimal getLjyqqs() {
		return this.ljyqqs;
	}

	public void setLjyqqs(BigDecimal ljyqqs) {
		this.ljyqqs = ljyqqs;
	}

	public String getShzt() {
		return this.shzt;
	}

	public void setShzt(String shzt) {
		this.shzt = shzt;
	}

	public String getSfzx() {
		return this.sfzx;
	}

	public void setSfzx(String sfzx) {
		this.sfzx = sfzx;
	}

	public String getKjhskmbh() {
		return this.kjhskmbh;
	}

	public void setKjhskmbh(String kjhskmbh) {
		this.kjhskmbh = kjhskmbh;
	}

	public BigDecimal getWdbh3() {
		return this.wdbh3;
	}

	public void setWdbh3(BigDecimal wdbh3) {
		this.wdbh3 = wdbh3;
	}

	public String getDklrlx() {
		return this.dklrlx;
	}

	public void setDklrlx(String dklrlx) {
		this.dklrlx = dklrlx;
	}

	public Set getSyDkZdhdlbs() {
		return this.syDkZdhdlbs;
	}

	public void setSyDkZdhdlbs(Set syDkZdhdlbs) {
		this.syDkZdhdlbs = syDkZdhdlbs;
	}

	public Set getSyDkYdtqsps() {
		return this.syDkYdtqsps;
	}

	public void setSyDkYdtqsps(Set syDkYdtqsps) {
		this.syDkYdtqsps = syDkYdtqsps;
	}

	public Set getGrzfdkjkhtxxes() {
		return this.grzfdkjkhtxxes;
	}

	public void setGrzfdkjkhtxxes(Set grzfdkjkhtxxes) {
		this.grzfdkjkhtxxes = grzfdkjkhtxxes;
	}

}