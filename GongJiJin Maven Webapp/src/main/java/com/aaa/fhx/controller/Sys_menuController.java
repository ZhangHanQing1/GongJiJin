...
import com.aaa.fhx.service.Sys_menuService;
import com.aaa.fhx.service.Sys_roleService;
...
public class Sys_menuController {
	...
	public String menusel(Sys_user s, Sys_user_role sur,HttpServletResponse response,HttpSession session,HttpServletRequest request,Map map) throws IOException{
		...
		System.out.println(s.getPasswords());
	...
	public PageInfo<Map<String, Object>> showUser(Integer startPage){
		PageHelper.startPage(startPage, 3);
		List<Map<String, Object>> list=sms.showUser();
	...
	public void add(Sys_user su,Sys_user_role sur,Sys_user_post sup,HttpServletResponse response) throws IOException{
		response.setContentType("text/html;charset=utf-8");
		PrintWriter out= response.getWriter();
		System.out.println(su.getUserId());
		System.out.println(sur.getRoleId());
		System.out.println(sup.getPostId());
		sms.addZong(su, sur, sup);	
	}
	...
	//修改权限表
		...
		public void updateRole(Sys_role ro,HttpServletResponse response) throws IOException{
			response.setContentType("text/html;charset=utf-8");
			PrintWriter out= response.getWriter();
			System.out.println(ro.getRoleId());
			sms.updateRole(ro);
		}
		//通过id查询角色表
		@ResponseBody
