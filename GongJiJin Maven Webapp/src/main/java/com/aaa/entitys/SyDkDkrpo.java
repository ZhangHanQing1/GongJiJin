package com.aaa.entitys;

import java.math.BigDecimal;
import java.util.Date;
import java.util.HashSet;
import java.util.Set;

/**
 * SyDkDkrpo entity. @author MyEclipse Persistence Tools
 */

public class SyDkDkrpo implements java.io.Serializable {

	// Fields

	private Integer pobh;
	private String podwzh;
	private String podwmc;
	private String podwdh;
	private String pogjjzh;
	private String poxm;
	private BigDecimal ponl;
	private String pojczt;
	private String pozjhm;
	private String pozjlx;
	private Double pozhye;
	private Date pojzny;
	private String cjr3;
	private Date cjsj3;
	private String posjhm;
	private String dkzh;
	private BigDecimal poysr;
	private String tzzd4;
	private String tzzd5;
	private Set grzfdkjkhtxxes = new HashSet(0);

	// Constructors

	/** default constructor */
	public SyDkDkrpo() {
	}

	/** full constructor */
	public SyDkDkrpo(String podwzh, String podwmc, String podwdh, String pogjjzh, String poxm, BigDecimal ponl,
			String pojczt, String pozjhm, String pozjlx, Double pozhye, Date pojzny, String cjr3, Date cjsj3,
			String posjhm, String dkzh, BigDecimal poysr, String tzzd4, String tzzd5, Set grzfdkjkhtxxes) {
		this.podwzh = podwzh;
		this.podwmc = podwmc;
		this.podwdh = podwdh;
		this.pogjjzh = pogjjzh;
		this.poxm = poxm;
		this.ponl = ponl;
		this.pojczt = pojczt;
		this.pozjhm = pozjhm;
		this.pozjlx = pozjlx;
		this.pozhye = pozhye;
		this.pojzny = pojzny;
		this.cjr3 = cjr3;
		this.cjsj3 = cjsj3;
		this.posjhm = posjhm;
		this.dkzh = dkzh;
		this.poysr = poysr;
		this.tzzd4 = tzzd4;
		this.tzzd5 = tzzd5;
		this.grzfdkjkhtxxes = grzfdkjkhtxxes;
	}

	// Property accessors

	public Integer getPobh() {
		return this.pobh;
	}

	public void setPobh(Integer pobh) {
		this.pobh = pobh;
	}

	public String getPodwzh() {
		return this.podwzh;
	}

	public void setPodwzh(String podwzh) {
		this.podwzh = podwzh;
	}

	public String getPodwmc() {
		return this.podwmc;
	}

	public void setPodwmc(String podwmc) {
		this.podwmc = podwmc;
	}

	public String getPodwdh() {
		return this.podwdh;
	}

	public void setPodwdh(String podwdh) {
		this.podwdh = podwdh;
	}

	public String getPogjjzh() {
		return this.pogjjzh;
	}

	public void setPogjjzh(String pogjjzh) {
		this.pogjjzh = pogjjzh;
	}

	public String getPoxm() {
		return this.poxm;
	}

	public void setPoxm(String poxm) {
		this.poxm = poxm;
	}

	public BigDecimal getPonl() {
		return this.ponl;
	}

	public void setPonl(BigDecimal ponl) {
		this.ponl = ponl;
	}

	public String getPojczt() {
		return this.pojczt;
	}

	public void setPojczt(String pojczt) {
		this.pojczt = pojczt;
	}

	public String getPozjhm() {
		return this.pozjhm;
	}

	public void setPozjhm(String pozjhm) {
		this.pozjhm = pozjhm;
	}

	public String getPozjlx() {
		return this.pozjlx;
	}

	public void setPozjlx(String pozjlx) {
		this.pozjlx = pozjlx;
	}

	public Double getPozhye() {
		return this.pozhye;
	}

	public void setPozhye(Double pozhye) {
		this.pozhye = pozhye;
	}

	public Date getPojzny() {
		return this.pojzny;
	}

	public void setPojzny(Date pojzny) {
		this.pojzny = pojzny;
	}

	public String getCjr3() {
		return this.cjr3;
	}

	public void setCjr3(String cjr3) {
		this.cjr3 = cjr3;
	}

	public Date getCjsj3() {
		return this.cjsj3;
	}

	public void setCjsj3(Date cjsj3) {
		this.cjsj3 = cjsj3;
	}

	public String getPosjhm() {
		return this.posjhm;
	}

	public void setPosjhm(String posjhm) {
		this.posjhm = posjhm;
	}

	public String getDkzh() {
		return this.dkzh;
	}

	public void setDkzh(String dkzh) {
		this.dkzh = dkzh;
	}

	public BigDecimal getPoysr() {
		return this.poysr;
	}

	public void setPoysr(BigDecimal poysr) {
		this.poysr = poysr;
	}

	public String getTzzd4() {
		return this.tzzd4;
	}

	public void setTzzd4(String tzzd4) {
		this.tzzd4 = tzzd4;
	}

	public String getTzzd5() {
		return this.tzzd5;
	}

	public void setTzzd5(String tzzd5) {
		this.tzzd5 = tzzd5;
	}

	public Set getGrzfdkjkhtxxes() {
		return this.grzfdkjkhtxxes;
	}

	public void setGrzfdkjkhtxxes(Set grzfdkjkhtxxes) {
		this.grzfdkjkhtxxes = grzfdkjkhtxxes;
	}

}